...
def home():
    ...
    for i in cal.itermonthdays(current_year, current_month):
        ...
        if int(i) == 0 :
            ...
            if calendar.weekday(current_year, current_month, i) == 5 or calendar.weekday(current_year, current_month, i) == 6:
                temp.append('weekend')
    ...
    return render_template('index.html', data=db_all, user=username, month=current_month_text, mon=current_month, year=current_year, status=status, total=int(total_week))
...
def user():
    if 'username' in session:
        ...
        with conn:
            cur = conn.cursor()
            sql = "SELECT * FROM `SEM` WHERE username=%s"
            cur.execute(sql, (user))
            all = cur.fetchall()
        return render_template('user.html', data=all, user=user)
    else:
        user = None
        return render_template('error.html')
